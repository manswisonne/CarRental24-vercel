...
const ManageBookings = () => {
  const { currency, axios } = useAppContext()
  const [bookings, setBookings] = useState([]) // ✅ Already safe with empty array
  ...
  const changeBookingStatus = async (bookingId, status) => {
    try {
      const { data } = await axios.post('/api/owner/change-booking-status', { 
        bookingId, 
        status 
      })
                  ...
                  <tr key={booking._id || index} className="border-t border-borderColor text-gray-500">
                    ...
                    <td className="p-3 max-md:hidden">
                      <span className={`py-1 px-3 rounded-full text-xs font-medium ${
                        ...
                        {booking.paymentStatus === 'paid' ? 'Paid' : 'Offline'}
                      </span>
                    </td>
                    ...
                    <td className="p-3">
                      {booking.status === "pending" ? (
                        <select 
                          ...
                          value={booking.status}
                          className="px-2 py-1.5 text-sm text-gray-600 border border-borderColor rounded-md outline-none focus:ring-2 focus:ring-blue-500"
                        >
                          <option value="pending">Pending</option>
                ...
                <tr>
                  <td colSpan="5" className="p-12 text-center">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <p className="text-gray-400 text-sm mt-1">Bookings will appear here once customers rent your cars</p>
                    </div>
                  </td>
                </tr>
              )}
